...
const CustomTooltip = ({ active, payload, label, selectedMetric }: CustomTooltipProps) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-slate-900/95 backdrop-blur-sm p-6 rounded-xl shadow-xl border border-slate-600/50">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm font-medium" style={{ color: getRiskColor(data.risk) }}>
            Riesgo: {data.risk}
          </p>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 to-slate-900 p-4 md:p-6">
      <Card className="bg-slate-900/90 backdrop-blur-sm border-none text-white shadow-lg">
        <CardHeader className="border-b border-slate-700/30 space-y-6 pb-6">
          <CardTitle className="text-4xl font-bold tracking-tight bg-gradient-to-r from-blue-300 to-purple-300 bg-clip-text text-transparent text-center pb-4">
            Proyecciones de Bitcoin Post-Halving
          </CardTitle>
          <div className="font-mono text-lg text-white text-center">
            Precio Actual: {formatCurrency(currentPrice)} | ATH: $108,200
          </div>
        </CardHeader>
        <CardContent>
          <div className="space-y-8 py-6">
            <div className="flex flex-wrap gap-4 justify-center">
              {[
                ...
                <button
                  key={metric}
                  className={`
                    flex items-center space-x-2 px-8 py-3 rounded-full
            ...
            <div className="h-[500px] lg:h-[550px] bg-slate-900/90 backdrop-blur-sm rounded-xl p-4 relative">
              <ResponsiveContainer width="100%" height="100%" minHeight={400}>
                <BarChart
                  data={scenarios}
                  margin={{ top: 40, right: 40, left: 60, bottom: 20 }}
                  barGap={12}
                  barSize={70}
                >
                  <defs>
